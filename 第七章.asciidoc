...
1. 棧為空；或者
1. 棧頂元素小於0；或者
1. 棧頂元素鎖定時間的類型（區塊高度或時間戳）與 +nLocktime+ 欄位不同；或者
...
具有小於 2^31^ 的 +nSequence+ 值的交易輸入被解釋為具有相對時間鎖。這種交易只有在輸入已經過相對時間鎖表示的時間後才有效。例如，具有 +nSequence+ 為30個塊的相對時間鎖的一個輸入的交易，僅在從輸入中引用的UTXO被開採的時間起，至少經過30個塊時才有效。由於 +nSequence+ 是每個輸入的欄位，交易可能包含任意數量的時間鎖定輸入，所有這些輸入都必須滿足時間要求交易才有效。一個交易可以同時包括時間鎖定的輸入（ +nSequence+ < 2^31^ ）和沒有時間鎖定的輸入（ +nSequence+ >= 2^31^ ）。
