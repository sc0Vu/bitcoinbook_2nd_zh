...
頻寬 :: 每筆交易都在網路上至少傳輸一次（通過泛洪傳播），如果在區塊傳播協議中沒有進行任何優化，交易將作為區塊的一部分再次傳輸，從而對網路容量的影響加倍。

記憶體 :: 驗證交易的節點將UTXO索引或整個UTXO集保存在內存中，以加快驗證。因為內存至少比硬碟貴一個數量級，所以UTXO集的增長不成比例地增加了運行節點的成本。
